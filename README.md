...
A child-first creative and learning world that grows with the child from **Grade 1 to Grade 6** (Philippines DepEd K-12): six learning islands — **Letter Lagoon** (tracing, phonics), **Number Cove** (counting, sorting, shapes, patterns), **Color & Art Bay** (drawing, coloring pages, sticker scenes, puzzles, music maker), **Science Shore** (animals, body, weather), **Story Reef** (read-along stories), **Craft Beach** (real crafts with a camera) — a **Daily Quest** with a treasure chest, diamonds and a 36-sticker album, plus **My Garden**, **Music Reef** and **My Diamond Book**. Every activity is data and carries its grade levels: hundreds of authored quizzes for Science, Araling Panlipunan, Filipino and English grammar, plus endless seeded Math (19 generators) and Words (6 modes) drills per grade. A parent sets the grade in Grown-ups → Learning; with auto-advance on, PJ moves up one grade when she masters the current one. New packs can be published to `public/content/packs.json` and the app picks them up at launch — download only, same origin only. Guided by **Jelly**, an original yellow jellyfish.
