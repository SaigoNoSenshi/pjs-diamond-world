...
- **Service worker + media.** Media elements request audio with a `Range` header, which the Cache API never matches against a stored full response; `sw.js` serves those as `206` slices from the cached file (otherwise sound fails offline). Only `200` responses are written to the cache.
- **Publishing constraint.** The GitHub connector used for deployment writes text files only, one commit per file, and cannot touch `.github/workflows/`. Hence `assets-mirror/` (base64 twins of every binary; `scripts/assets-mirror.mjs`, decoded on `npm install`, checked in `validate`/CI). Change an asset → run `npm run assets:encode` before committing.
...
## 17. Learning platform (Plan 3)

- **Activities are data.** `domain/activity/schema.ts` defines `ActivityDefinition` (id, kind, island, title, icon, colour, voice intro, instruction, reward, `data`) with a discriminated `data` union per engine (TRACE, QUIZ, COUNTING, MATCHING, PUZZLE, COLORING, STICKER_SCENE, STORY, MUSIC_MAKER, DRAW, CRAFT). `content/activities/index.ts` parses every pack at startup; a bad entry fails tests, never a child. Pictures are `{ icon, color?, text? }` rendered by `PictureView`; icons come from `components/icons/extraIcons.ts` (data specs on a 100×100 grid) through the existing `Icon`.
- **Engines** live in `features/activities/engines/*` and implement `EngineProps` (`activity`, `onComplete(score?)`). `ActivityPlayerScreen` resolves the id, plays the engine, then calls `LearningProvider.complete()` and shows the reward celebration. DRAW/CRAFT redirect to the existing screens and are completed from the event bus when the drawing/craft is really saved.
- **Learning progress** (`domain/learning/*`): `LearningProgress` (diamonds, stickers, completions, quest) with pure `recordCompletion` and `ensureQuest`/`pickDailyQuest`; stored by `LearningProgressRepository` (memory, IndexedDB store `learningProgress` v2, SQLite `learning_progress` migration v2). Rewards are additive only.
- **Islands** (`content/islands.ts`), **stickers** (`content/stickers.ts`), **notes** (`content/music/notes.ts` → `NoteService`/`ExpoNotePlayer`).
- **Routes:** `/island/[islandId]`, `/play/[activityId]`, `/quest`, `/stickers` (all lazy; the engines share one chunk).
- **Adding content:** add an object to the right pack (or a new pack listed in `content/activities/index.ts`), an icon spec if needed, and run `npm run validate`. Illustrations: WebP ≤ 512 px in `assets/images/**`, register in `constants/images.ts`, then `npm run assets:encode`.

## 18. Grades 1–6 and content updates (Phase 18)

- **Grade levels.** Every `ActivityDefinition` carries `grades: number[]` (1–6, DepEd K-12) and a `subject` (english, math, science, filipino, araling_panlipunan, art, music, craft). The child profile stores `grade` and `autoAdvanceGrade` (`domain/profile/schema.ts`); a parent sets the grade in **Grown-ups → Learning**. Islands (`activitiesForIsland(islandId, grade)`), the home progress ring, the daily quest and the sticker rewards all filter on the current grade; grade chips on an island let PJ revisit earlier grades, never peek ahead.
- **Mastery and auto-advance** (`domain/learning/grades.ts`): a grade is mastered when ≥ 80 % of its _core-subject_ activities (English, Math, Science, Filipino, AP) have been completed with a best score ≥ 0.8 (minimum 5). On completing an activity `LearningProvider.complete()` calls `nextGrade()`; with auto-advance on, the grade rises by exactly one level and the player shows a "level up" celebration. The grade never moves down automatically. Pure, tested, no timers.
- **Endless generated content.** `domain/generators/math.ts` — 19 seeded generators (addition … word problems, ratios, exponents, volume) with the grades each covers and a `keypadFrom` grade after which the answer is typed rather than chosen; `domain/generators/words.ts` — spell / missing letter / unscramble / synonym / antonym / part of speech built from `content/words/grade1..6.ts` (30 words, 12 sentences, pairs and POS entries per grade). `content/activities/generated.ts` expands them into one MATH activity per generator × grade and one WORDS activity per mode × grade (`act_math_<generator>_g<N>`, `act_words_<mode>_g<N>`). Every play is a new seed, so the banks never run out. Engines: `MathPlayer` (choice cards or `Keypad`, two misses reveal the answer) and `WordsPlayer` (tile order, missing letter, choose).
- **Authored banks.** `content/activities/quizGrades.ts` — Science and Araling Panlipunan, two sets per grade; `content/activities/quizLanguage.ts` — Filipino (two sets per grade) and English grammar. Existing packs carry explicit grades (stories 1–2, first-wave quizzes 1–2, crafts 1–3, art/music 1–6).
- **Registry** (`content/activities/registry.ts`): activities are registered per _source_ (`builtin`, `remote`) and can be replaced or withdrawn atomically; `registryVersion`/`subscribeActivities` let screens (`useActivities()`) re-render when content changes. Built-in packs are registered strictly (a bad entry fails tests); remote entries are validated individually and the bad ones dropped.
- **Content updates — auto-delivered, never auto-generated.** `services/content/RemoteContentService` fetches `<app origin>/<base>/content/packs.json` (web: **same origin only**, i.e. the GitHub Pages site that serves the app; native: `EXPO_PUBLIC_CONTENT_URL`, unset by default) at launch and on **Grown-ups → Learning → Check now**. The pack (`{ version, publishedAt, note?, activities[] }`) is validated with the same zod schema, merged under the `remote` source, and cached in AsyncStorage so it works offline next time; a newer version replaces the previous one entirely (withdrawn activities disappear, progress on them is kept). Nothing is ever sent — the request has no query string, no identifiers, no cookies. The service worker treats `/content/*` as network-first with cache fallback. New content is authored and committed by an adult (or a scheduled authoring agent) to `public/content/packs.json`; `src/content/__tests__/packsJson.test.ts` validates it on every build so a broken pack cannot ship.
